...
**预构建**是 Vite 在开发服务器启动前，使用 esbuild 预先对第三方依赖（`node_modules` 中的包）进行转换和打包的过程。

### 预构建的两个核心作用

1. **兼容性转换**：将 CommonJS / UMD 格式转换为 ESM 格式
2. **性能优化**：将分散的小文件合并，减少 HTTP 请求数量
...
预构建结果会保存到 `node_modules/.vite/` 目录：

```
node_modules/.vite/
├── _metadata.json       # 预构建元数据
├── lodash.js            # 预构建后的 lodash
...
### 1.1 问题 1：CommonJS/UMD 兼容性【兼容转换】
...
## 2. CommonJS/UMD → ESM 转换

### 2.1 CommonJS 转 ESM 示例
...
```json
{
  ...
  "deps": {
    ...
    "lodash": {
      "file": "node_modules/.vite/lodash.js",
...
| 字段 | 类型 | 作用 |
|------|------|------|
| `hash` | `string` | 缓存哈希值，用于判断是否需要重新预构建 |
| `optimizer.esbuildVersion` | `string` | 使用的 esbuild 版本，版本变化时重新构建 |
| `browserHash` | `string` | 浏览器端使用的哈希值，用于缓存控制 |
| `deps` | `object` | 预构建依赖的映射表 |
| `deps[depName].file` | `string` | 预构建后的文件路径 |
| `deps[depName].src` | `string` | 原始依赖的源文件路径 |
| `deps[depName].needsInterop` | `boolean` | 是否需要 ESM 互操作（CommonJS 转 ESM） |
| `deps[depName].fileHash` | `string` | 预构建文件的内容哈希 |
